...
Emakefun_DCMotor *g_dc_motor_0 = g_motor_driver.getMotor(M1);
Emakefun_DCMotor *g_dc_motor_1 = g_motor_driver.getMotor(M2);
Emakefun_DCMotor *g_dc_motor_2 = g_motor_driver.getMotor(M3);
Emakefun_DCMotor *g_dc_motor_3 = g_motor_driver.getMotor(M4);
...
void setup() {
  ...
  g_dc_motor_0->run(BRAKE);
  g_dc_motor_1->run(BRAKE);
  g_dc_motor_2->run(BRAKE);
  g_dc_motor_3->run(BRAKE);
...
void loop() {
  ...
  if (millis() - g_motor_last_update_time >= kMotorUpdateIntervalMs) {
    if (it.holding(Button::kCrossA)) {
      ...
      g_dc_motor_0->setSpeed(kMotorRunSpeed);
      g_dc_motor_0->run(FORWARD);
      g_dc_motor_1->setSpeed(kMotorRunSpeed);
      g_dc_motor_1->run(FORWARD);
      g_dc_motor_2->setSpeed(kMotorRunSpeed);
      g_dc_motor_2->run(FORWARD);
      g_dc_motor_3->setSpeed(kMotorRunSpeed);
      g_dc_motor_3->run(FORWARD);
      g_motor_last_update_time = millis();
    } else if (it.holding(Button::kDown) && !it.holding(Button::kUp)) {
      // 下方向键：电机反转 | Down button: move backward.
      g_dc_motor_0->setSpeed(kMotorRunSpeed);
      g_dc_motor_0->run(BACKWARD);
      g_dc_motor_1->setSpeed(kMotorRunSpeed);
      g_dc_motor_1->run(BACKWARD);
      g_dc_motor_2->setSpeed(kMotorRunSpeed);
      g_dc_motor_2->run(BACKWARD);
      g_dc_motor_3->setSpeed(kMotorRunSpeed);
      g_dc_motor_3->run(BACKWARD);
      g_motor_last_update_time = millis();
    } else {
